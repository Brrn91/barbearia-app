...
function gerarTodosHorarios(): string[] {
...
async function listarHorariosDisponiveis(
  ...
  resultado.rows.forEach((agendamento) => {
    ...
    const duracaoMinutos: number = agendamento.duracao_minutos;

    for (
      let minutos = inicioMinutos;
      minutos < inicioMinutos + duracaoMinutos;
...
export { listarHorariosDisponiveis };
